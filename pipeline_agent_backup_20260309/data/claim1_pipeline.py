...
print("=" * 60)
print("DAG       : dag_claim1")
print("Table     : hadoop_prod.pipeline_db.claim1")
print("Warehouse : hdfs://localhost:8020/new")
print("Run time  : " + str(datetime.now()))
print("=" * 60)

# Step 1: Read current data
df = spark.sql("SELECT * FROM hadoop_prod.pipeline_db.claim1")
count = df.count()
...
df.writeTo("hadoop_prod.pipeline_db.claim1") \
...
snapshots = spark.sql("""
    ...
    FROM hadoop_prod.pipeline_db.claim1.snapshots
...
spark.sql("SELECT * FROM hadoop_prod.pipeline_db.claim1 LIMIT 10").show(truncate=False)

new_count = spark.sql(
    "SELECT COUNT(*) as n FROM hadoop_prod.pipeline_db.claim1"
).collect()[0]['n']
